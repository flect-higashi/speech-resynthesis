...
class CodeGenerator(Generator):
    def __init__(self, h):
        ...
        if h.get('f0_quantizer_path', None):
            ...
            quantizer_state = torch.load(
                h.f0_quantizer_path, map_location='cpu', weights_only=False)
    ...
    def _upsample(signal, max_frames):
        ...
        reminder = (max_frames - signal.shape[2]
                    * signal.shape[3]) // signal.shape[3]
        if reminder > 0:
    ...
    def forward(self, **kwargs):
        ...
        if self.code_vq and kwargs['code'].dtype is torch.int64:
            ...
            _, code_h_q, code_commit_losses, code_metrics = self.code_vq(
                code_h)
            x = code_h_q[0]
        else:
